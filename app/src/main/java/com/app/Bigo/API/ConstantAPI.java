package com.app.Bigo.API;

/**
 * Created by nguyennam on 2/11/17.
 */

public class ConstantAPI {

    public static final int TIME_OUT = 2000;

    public static final String SERVER = "http://static.letsbigo.com/";
    // ------ list API --------
    public static final String API_LISTAPI = "http://static.letsbigo.com/list.json";

    public static final String API_LIST_TOP = "http://static.letsbigo.com/top.json";
    public static final String API_LIST_NEW = "http://static.letsbigo.com/new.json";
    public static final String API_LIST_ALL = "http://static.letsbigo.com/all.json";

    // ------ list fanpage ------------
    public static final String FACEBOOK_PAGE = "https://www.facebook.com/letsbigo";
    public static final String YOUTUBE_CHANNEL = "https://www.youtube.com/c/Letsbigo";

}
